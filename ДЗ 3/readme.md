...
## В этой задаче рассмотрим сеть ( 172.22.32.12/19 )
  
   |Значение 172.22.32.12 в двоичной системе будет равно         | 10101100.00010110.00100000.00001100 |
   |-------------------------------------------------------------|-------------------------------------|
   |Значение маски 19 в двоичной системе будет равно             | 11111111.11111111.11100000.00000000 |
   |Если перемножим каждое напротив стоящие значени то получим cетевой адрес подсети (Network) (В данном приере первые два октета буду повторятся с адресом сети тк в маске все единицы и умножая на 1 получим исходное значение. А вторые два октета получат значения ) | 10101100.00010110.00100000.00000000 | 
   |Теперь можем узнать широковещательный IPv4 подсети (broadcast), где первые биты (напротив маски) останутся неизенны до условно заканчивающихся едениц в маске, а последующие биты будут все еденицы   | 10101100.00010110.00111111.11111111 |
   ...
   | 10101100.00010110.00100000.00001100 | 172.22.32.12    |                        | адрес и значение исходной сети |
   |-------------------------------------|-----------------|------------------------|--------------------------------|
   | 11111111.11111111.11100000.00000000 | 255.255.224.0     | можно посмотреть по таблице или вычислить по октетно 255=октету из восьми единиц и тд | маска подсети (в данном примере 19 бит) |
   | 10101100.00010110.00100000.00000000 | 172.22.32.0      | сопоставляя или умножая двоичные значения  | идентификатор сети |
   | 10101100.00010110.11111111.11111111 | 172.22.63.255  | max допустимое значение бит в сети (тк октет max все единицы в десятиричной системе = 255)  | broadcast |
   | 10101100.00010110.00100000.00000001 | 172.22.32.1      | network +1             | host min  |
   | 10101100.00010110.00111111.11111110 | 172.22.63.254  | broadcast -1           | host max  |

## В этой задаче рассмотрим исходную сеть ( 172.22.32.12/16 )
  
   |Значение 172.22.32.12 в двоичной системе будет равно         | 10101100.00010110.00100000.00001100 |
   |-------------------------------------------------------------|-------------------------------------|
   |Значение маски 16 в двоичной системе будет равно             | 11111111.11111111.00000000.00000000 |
   |Если перемножим каждое напротив стоящие значени то получим cетевой адрес подсети (Network) (В данном приере первые два октета буду повторятся с адресом сети тк в маске все единицы и умножая на 1 получим исходное значение. А вторые два октета обнулятся тк в маске все нули ) | 10101100.00010110.00000000.00000000 | 
   |Теперь можем узнать широковещательный IPv4 подсети (broadcast), где первые биты (напротив маски) останутся неизенны до последних нулей, а последующие биты будут все еденицы   | 10101100.00010110.11111111.11111111 |
   ...
   | 10101100.00010110.11111111.11111111 | 172.22.255.255  |
   | 10101100.00010110.00000000.00000000 | 172.22.0.0      |  
   
   Теперь когда знаем network & broadcast можем вычислить последний host и первый.
   
   | 10101100.00010110.00100000.00001100 | 172.22.32.12    |                        | адрес и значение исходной сети |
   |-------------------------------------|-----------------|------------------------|--------------------------------|
   | 11111111.11111111.00000000.00000000 | 255.255.0.0     | можно посмотреть по таблице или вычислить по октетно 255=октету из восьми единиц и тд | маска подсети (в данном примере 16 бит) |
   | 10101100.00010110.00000000.00000000 | 172.22.0.0      | сопоставляя или умножая двоичные значения  | идентификатор сети |
   | 10101100.00010110.11111111.11111111 | 172.22.255.255  | max допустимое значение бит в сети (тк октет max все единицы в десятиричной системе = 255)  | broadcast |
   | 10101100.00010110.00000000.00000001 | 172.22.0.1      | network +1             | host min  |
   | 10101100.00010110.11111111.11111110 | 172.22.255.254  | broadcast -1           | host max  |
...
## Найти:
  ...
  |   IPv4 последнего узла подсети (host max)       |          192.135.250.182           | 
  |   Широковещательный IPv4 подсети (broadcast)    |          192.135.250.183           | 
